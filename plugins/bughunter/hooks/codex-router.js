...
const ROUTING = [
  ...
  'Run the fast submit_review first – unless the user asked for the deep hunt in their own words (deep, deep hunt, full-repo, --deep, in any language): then one submit_review with deep: true and meta.repo + meta.ref (the pushed head sha) + meta.base_branch, no payload, is the first and only review, with no fast stage first and no second question. Let the heartbeat wait at the server\'s cadence until the review is terminal, then read review_report.',
...
function finish() {
  ...
  if (/\b(?:review|reviewing|bug.?hunt|hunt\s+bugs?|deep\s+(?:review|hunt)|full[- ]repo\w*|pull\s+request|\bpr\b|merge)\b/i.test(prompt)) {
    ...
    // prompt carries them, so a plain "review this" never reads as a deep ask.
    const deep = /\bdeep\b|\bfull[- ]repo|--deep/i.test(prompt)
