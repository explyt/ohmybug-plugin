...
const ROUTING = [
  ...
  'The submit answer and every poll body carry live_url: the private page where the person who started the hunt watches it in a browser – stages, findings, the sandbox-destroyed receipt, a Save as PDF button – for 6 hours after the result. Print it once for the user as a plain line beside your terminal done/failed line. It is a capability link: never paste it into a PR, an issue, a commit or anything shared, and never poll it yourself – status_url is your door. No live_url on the body (an older server) means no page: say nothing about one.',
