...
const ROUTING = [
  ...
  'Run the fast submit_review first – unless the user asked for the deep hunt in their own words (deep hunt, deep review, full-repo, --deep, in any language – the adjective alone, as in deep-dive review or a deep look, asks for a thorough fast review): then one submit_review with deep: true and meta.repo + meta.ref (the pushed head sha) + meta.base_branch, no payload, is the first and only review, with no fast stage first and no second question. Let the heartbeat wait at the server\'s cadence until the review is terminal, then read review_report.',
...
function finish() {
  ...
  if (/\b(?:review|reviewing|bug.?hunt|hunt\s+bugs?|deep\s+(?:review|hunt)|full[- ]repo\w*|pull\s+request|\bpr\b|merge)\b/i.test(prompt)) {
    ...
    // ask. The noun phrase, not the adjective: a hyphen is a word boundary, so
    // \bdeep\b took "deep-dive review" and "the deep copy" for the paid hour.
    const deep = /\bdeep\s+(?:hunt|review|scan|pass|mode)\b|\bfull[- ]repo|--deep\b/i.test(prompt)
